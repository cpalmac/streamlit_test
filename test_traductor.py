...
#traduccion = translator.translate("Hello world", dest='es')
...
def buscarPelicula(data=None,genero=None,pais=None):
    ...
    if st.button("Buscar"):
        ...
        if pais != "Seleccione":
            ...
            for d in data_filtro["DESCRIPCION"]:
                #st.write(d)
                traduccion = translator.translate(d, dest='es')
                data_filtro["DESCRIPCION"] = traduccion.text
                i=i+1
